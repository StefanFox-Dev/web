...
export class PlayerTranslate {
  private static readonly translations: Record<Language, Record<string, string>> = {
    ru: {
  ...
  public static translate(key: string, ...params: (string | number)[]): string {
  ...
  public static toggleLanguage(): void {
    ...
    const next: Language = current === 'ru' ? 'uk' : current === 'uk' ? 'en' : 'ru';
    this.setLanguage(next);
  }

  public static updateDom(): void {
