...
class Application {
  private particles: ParticleEngine | null = null;
  private statusComp: ServerStatusComponent | null = null;
  private ranksComp: RanksCatalogComponent | null = null;
  private staffComp: StaffBoardComponent | null = null;
  private rulesComp: RulesViewerComponent | null = null;
  private skinComp: SkinViewerComponent | null = null;
  ...
  public init(): void {
    ...
    appStore.subscribe(
      (s) => s.activeTab,
      (tab) => this.switchTabPane(tab)
    );
  }

  private setupNavigation(): void {
    const tabButtons = document.querySelectorAll<HTMLButtonElement>('.mc-tab-btn');
    ...
    const closeBtn = document.querySelector<HTMLButtonElement>('.mc-btn-close');
    if (closeBtn) {
  ...
  private switchTabPane(targetId: string): void {
    const tabButtons = document.querySelectorAll<HTMLButtonElement>('.mc-tab-btn');
    for (const btn of tabButtons) {
    ...
    const panes = document.querySelectorAll<HTMLElement>('.mc-pane');
  ...
  public destroy(): void {
    ...
    if (this.statusComp) this.statusComp.destroy();
    if (this.ranksComp) this.ranksComp.destroy();
    if (this.staffComp) this.staffComp.destroy();
    if (this.rulesComp) this.rulesComp.destroy();
    if (this.skinComp) this.skinComp.destroy();
