...
export class RanksCatalogComponent {
    ...
    unsubscribe = null;
    constructor() {
        this.unsubscribe = appStore.subscribe((s) => s.language, () => this.loadRanks());
        this.loadRanks();
    }
    async loadRanks() {
        try {
    ...
    selectRank(rank) {
        ...
        const input = document.getElementById('donate-rank-select');
        if (input) {
            input.value = rank.id;
        }
    }
...
//# sourceMappingURL=ranks-catalog.js.map