...
export class RulesViewerComponent {
    ...
    searchHandler = null;
    constructor() {
        this.unsubscribe = appStore.subscribe((s) => s.language, () => this.loadRules());
        this.loadRules();
        this.setupSearch();
    }
    async loadRules() {
    ...
    setupSearch() {
        this.searchInput = document.getElementById('rules-search-input');
        if (this.searchInput) {
    ...
    filterRules(query) {
        ...
        const items = document.querySelectorAll('.mc-rule-section');
...
//# sourceMappingURL=rules-viewer.js.map